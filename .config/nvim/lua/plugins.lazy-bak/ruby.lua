return {
	{
		...
		init = function()
			...
			local function ensure_ruby(bufnr, filepath)
				...
				-- Check if dependencies are installed before starting ruby-lsp
				local gemfile = util.path.join(root, "Gemfile.lock")
				if not vim.loop.fs_stat(gemfile) then
					vim.schedule(function()
						pcall(vim.notify,
							string.format(
								"Ruby LSP: Gemfile.lock not found in %s. Run 'dev up' first.",
								project_label(root)
							),
							vim.log.levels.WARN,
							{ title = "Ruby LSP" }
					...
					on_exit = function(code, signal, client_id)
						if code == 1 then
							vim.schedule(function()
								pcall(vim.notify,
									string.format(
										"Ruby LSP exited with error in %s. Try running 'dev up'.",
										project_label(root)
									),
			...
			local function ensure_sorbet(bufnr, filepath)
				...
				-- Check if dependencies are installed before starting Sorbet
				local gemfile = util.path.join(root, "Gemfile.lock")
				if not vim.loop.fs_stat(gemfile) then
					vim.schedule(function()
						pcall(vim.notify,
							string.format(
								"Sorbet: Gemfile.lock not found in %s. Run 'dev up' first.",
								project_label(root)
							),
							vim.log.levels.WARN,
							{ title = "Sorbet LSP" }
					...
					on_exit = function(code, signal, client_id)
						if code == 1 then
							vim.schedule(function()
								pcall(vim.notify,
									string.format(
										"Sorbet exited with error in %s. Try running 'dev up'.",
										project_label(root)
									),
									vim.log.levels.ERROR,
									{ title = "Sorbet LSP" }
